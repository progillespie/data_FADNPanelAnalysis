...
<<echo=TRUE, results='markup'>>=
# create two new data tables for just the 2005 year
nfs05 <- nfs[year==2005]  
...
nfskey <- nfskey[sr:er, list(ENTREPRISE.LAYOUT, X)] # select starting to ending row, cols 1 to 2 (note that the typo in "ENTREPRISE.LAYOUT" is in the csv, hence it is "correct" here)
setnames(nfskey, c("ENTREPRISE.LAYOUT", "X"),c("labels", "varnames"))  # rename columns
nfskey <- nfskey[varnames!=""]
fadn.names <- names(fadn05)
viewData(nfskey)
@
