...
# reads each line of code in FarmRegrExpVars.do (part of IGM, 1 line per cell)
FarmRegr <- data.table(lines = 
      scan("/media/MyPassport/IGM/FarmRegrExpVars.do"
      , what=character()
      , comment.char="*"
      , sep="\r"
      )
...
# lines of FarmRegrExpVars.do for a call to that variable (returning a 
...
# column in nfs.dict called InFarmRegr and write out a new csv.
nfs.dict[Variable.Name!=""
     , InFarmRegr:= ifelse(
          length(data.table(grepl(Variable.Name, FarmRegr[,lines]))[V1==TRUE,V1])>0
          ,"y"
          ,"n"
          )
,by=Variable.Name]
