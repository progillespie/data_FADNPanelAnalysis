...
This version:
  ...
  - A long run cost model (incomplete)
  - Adding countries (incomplete)
  - Better mapping of zones (incomplete)
...
global 	ms 		"Germany France Ireland UnitedKingdom Italy Poland" //
...
sfpanel `dep_vlist' `indep_vlist' TREND if country == "ITA",  `cost' model(bc95) d(tnormal) posthessian robust emean(`z_vlist') usigma(`z_vlist')
estimates store `model1name'_ITA_`tech_or_cost'
estimates save `fadnoutdatadir'/`model1name'_ITA_`tech_or_cost'_$datestamp$timestamp.ster, replace
di `e(converged)'

sfpanel `dep_vlist' `indep_vlist' TREND if country == "POL",  `cost' model(bc95) d(tnormal) posthessian robust emean(`z_vlist') usigma(`z_vlist')
estimates store `model1name'_POL_`tech_or_cost'
estimates save `fadnoutdatadir'/`model1name'_POL_`tech_or_cost'_$datestamp$timestamp.ster, replace
di `e(converged)'
