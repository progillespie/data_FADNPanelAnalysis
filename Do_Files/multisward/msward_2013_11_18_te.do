...
This version:
  ...
  - Estimate common frontier (TE) across all countries
  - Use data from OrigData/FADN_From_Fiona
  - Use improved grass var by weighting fdratio by
	(K147AA + K150AA + K151AA)/SE071
    ... which is just proportion of grass area in
        forage area
  - Will do the same for zones in separate file
*******************************************************
*******************************************************/
...
*local 	z_vlist		"`z_vlist' ogagehld"
*local 	z_vlist		"`z_vlist' special"
*local 	z_vlist		"`z_vlist' intense"
...
/* Supply appropriate vlist for your model (e.g. dep_vlist "lnq_vlist"
    indep_vlist "`lnx_vlist for a production frontier) */
local 	dep_vlist 	"`lnc_vlist'" // using an input index doesn't seem to work
...
if $databuild == 1{
	...
	* Check if altitudezone exists in data
	capture confirm altitudezone, exact
	...
	* Check if grassregion exists in data
	capture confirm grassregion, exact
	...
	foreach lnvar of local CobbDouglas {
		...
		foreach lnvar2 of local interactions {
			...
			* Compile in a handy list
			local translog_terms "`translog_terms' half_lnTL_`i'`j'"
		...
		rename `var' 	CD_`i' 
		rename `lnvar'	lnCD_`i'
		local primary_terms "`CobbDouglas_terms' CD_`i'"
		local interactions: list interactions - lnvar
	}
	
	
	local translog		"`CobbDouglas_terms' `translog_terms'"
	local indep_vlist "``func_form''"
	macro list _CobbDouglas_terms _translog_terms _translog _func_form _indep_vlist
...
capture drop intwt
gen fdratio = 100*(feedforgrazinglivestockhomegrown/feedforgrazinglivestock)
gen forage_sh = foragecropsuaa/totaluaa
*(K147AA + K150AA + K151AA)/SE071
gen grass_sh = (temporarygrassaa+meadowpermpastaa+roughgrazingaa)/foragecropsuaa
gen grassratio = fdratio * grass_sh 
gen special    = dpallocgo
gen intense    = dpnolu/daforare
...
* MODEL COMMANDS HERE
* ===================
sfpanel `dep_vlist' `indep_vlist' `location_vlist' TREND if country == "DEU", model(bc95) cost d(tnormal)  emean(`z_vlist')  usigma(`z_vlist')  
estimates store `model1name'_DEU_tech

sfpanel `dep_vlist' `indep_vlist' `location_vlist' TREND if country == "FRA", model(bc95) cost d(tnormal)  emean(`z_vlist')  usigma(`z_vlist') vsigma(`z_vlist')
estimates store `model1name'_FRA_tech
 
sfpanel `dep_vlist' `indep_vlist' `location_vlist' TREND if country == "IRE", model(bc95) cost d(tnormal)  emean(`z_vlist')  usigma(`z_vlist')  
estimates store `model1name'_IRE_tech

sfpanel `dep_vlist' `indep_vlist' `location_vlist' TREND if country == "UKI", model(bc95) cost d(tnormal)  emean(`z_vlist')  usigma(`z_vlist')
estimates store `model1name'_UKI_tech

sfpanel `dep_vlist' `indep_vlist' `location_vlist' TREND if atl_plains == 1, model(bc95) cost d(tnormal)  emean(`z_vlist')  usigma(`z_vlist')  vsigma(`z_vlist')
estimates store `model1name'_ATL_tech

sfpanel `dep_vlist' `indep_vlist' `location_vlist' TREND if cont_eur == 1, model(bc95) cost d(tnormal)  emean(`z_vlist')  usigma(`z_vlist')  vsigma(`z_vlist')
estimates store `model1name'_CEU_tech

sfpanel `dep_vlist' `indep_vlist' `location_vlist' TREND, model(bc95) cost d(tnormal)  emean(`z_vlist')  usigma(`z_vlist')  vsigma(`z_vlist')
estimates store `model1name'_ALL_tech
