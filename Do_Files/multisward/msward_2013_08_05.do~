...
local 	indep_vlist 	"lndotomkgl "
local 	indep_vlist 	"`indep_vlist' lnPLand" 
local 	indep_vlist 	"`indep_vlist' lnPTotalCattle" 
*local 	indep_vlist 	"`indep_vlist' lnPMinAgWage" 
local 	indep_vlist 	"`indep_vlist' lnPLabour" 
*local 	indep_vlist 	"`indep_vlist' lnPVetExp" 
*local 	indep_vlist 	"`indep_vlist' lnPElectricity" 
*local 	indep_vlist 	"`indep_vlist' lnPCattleFeed" 
*local 	indep_vlist 	"`indep_vlist' lnPMotorFuels" 
*local 	indep_vlist 	"`indep_vlist' lnPTotalFert" 
*local 	indep_vlist 	"`indep_vlist' lnPOtherInputs" 
...
local 	model1command 	"sfpanel `dep_vlist' `indep_vlist' `location_vlist' i.countrycode2 i.year countrycode2#year if country=="IRE" | country=="UKI", model(bc95) cost d(tnormal) emean(`z_vlist', noconstant)  usigma(`z_vlist', noconstant) vsigma(`z_vlist', noconstant)"
...
global timestamp: di  %tdCCYY!_NN!_DD date("$S_DATE", "DMY")
...
qui estimates save  `fadnoutdatadir'/`model1name'$datestamp$S_TIME.ster, replace
...
tw (sc te_`modelname' fdratio) (lfit `modelname' fdratio, lwidth(thick))  if country=="IRE"
