...
local 	z_vlist		"`z_vlist' grassratio"
local 	z_vlist		"`z_vlist' TREND"
...
global 	ms 		"Ireland France Germany UnitedKingdom" //
...
if $databuild == 1{
	...
	*/
	/* 
	Atlantic plains,
        - France (Brittany) (IE): Forage crops region: temporary grasslands + maize;               -       
	- France (Normandy) (IE): Permanent grassland and maize region (‘Bocage’);
        - UK (Wales) (AU-IBERS)
	- Ireland (TEAGASC): Grassland region: permanent grassland dominant in the Agricultural Area (AA).
 
       Humid mountains,
        - Switzerland (Agroscope, FiBL): Wet mountain region. [you wont have the data for this one but I explained this to Alain back in September]
 
        Mediterranean zones,
        - Italy (UNIUD): Mediterranean regions in fertile plains and valleys as well as in dry mountain rangelands.
	...
	* grass proxy
	...
	gen grassratio = fdratio * forage_sh
...
gen TREND =year-1998
	...
	* grass proxy
	capture drop fdratio forage_sh lnfdratio intwt
	gen fdratio = 100*(feedforgrazinglivestockhomegrown/feedforgrazinglivestock)
	gen forage_sh = foragecropsuaa/totaluaa
	gen grassratio = fdratio * forage_sh * 100
