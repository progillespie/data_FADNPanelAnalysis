...
sfpanel `dep_vlist' `indep_vlist' TREND if country == "FRA", `cost' model(bc95) d(tnormal) posthessian robust emean(`z_vlist') usigma(TREND) vsigma(TREND) 
estimates store `model1name'_FRA_`tech_or_cost'
di `e(converged)'

sfpanel `dep_vlist' `indep_vlist' TREND if country == "IRE", `cost' model(bc95) d(tnormal) posthessian robust emean(`z_vlist') usigma(TREND) vsigma(TREND)
estimates store `model1name'_IRE_`tech_or_cost'
di `e(converged)'

sfpanel `dep_vlist' `indep_vlist' TREND if country == "UKI", `cost' model(bc95) d(tnormal) posthessian robust emean(`z_vlist') usigma(TREND) vsigma(TREND)
estimates store `model1name'_UKI_`tech_or_cost'
di `e(converged)'

sfpanel `dep_vlist' `indep_vlist' TREND if atl_plains == 1,  `cost' model(bc95) d(tnormal) posthessian robust emean(`z_vlist') usigma(TREND) vsigma(TREND)
estimates store `model1name'_ATL_`tech_or_cost'
di `e(converged)'

sfpanel `dep_vlist' `indep_vlist' TREND if cont_eur == 1,    `cost' model(bc95) d(tnormal) posthessian robust emean(`z_vlist') usigma(TREND) vsigma(TREND)
estimates store `model1name'_CEU_`tech_or_cost'
di `e(converged)'
*/
*sfpanel `dep_vlist' `indep_vlist' France Ireland Wales TREND , cost model(bc95) d(tnormal) posthessian robust emean(`z_vlist') vsigma(TREND)
* BEST RESULT Effect on Mu parameter = -0.0054 Sigma U Parameter = 2.308268 need to figure out partial effect for this. Non-monotonic effect of grass.
...
* Second to last run.... above is better.
*sfpanel lntotalspecificcosts lndairyproducts lnPTotalCattle lnPCompoundfeedingstuffsforcatt lnPFERTILISERSANDSOILIMPROVERS TREND , cost model(bc95) d(tnormal) posthessian robust emean(lngrassratio) usigma(lngrassratio) vsigma(TREND)
