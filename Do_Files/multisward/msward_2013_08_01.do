...
macro list _indep_vlist _location_vlist
STOP!
*/
...
local 	model1command 	"sfpanel `dep_vlist' `indep_vlist' `location_vlist' if country=="IRE" , model(tfe) cost d(tnormal) emean(`z_vlist', noconstant)  usigma(`z_vlist', noconstant) vsigma(`z_vlist', noconstant)"
...
* Gen 
capture gen PLand = rentpaid/renteduaa
capture gen PLabour = wagespaid/flabpaid
...
qui estimates save  `model1name'$datestamp.ster, replace
