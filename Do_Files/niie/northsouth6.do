...
/* If selecting just BMW in Ireland, then stick "bmw" in logfile names. 
    Now you won't overwrite logs for whole Ireland dataset. */
if `BMWonly'==1 {
	local bmwarg = "bmw"
}
...
do sub_do/cex_vargen.do      `bmwarg' 	// creates variables cex_ do-files
do sub_do/cex_decomp.do      `bmwarg' 	// cex means 
do sub_do/cex_decomp_lt.do   `bmwarg'  	//  "Constant exchange rate adjustment"
do sub_do/cex_decomp_lu.do   `bmwarg'  	//
do sub_do/cex_decomp_ha.do   `bmwarg'  	// decomp means 
...
* ROI vs. NI
	* NM Kernel Density ROI- NI (adjusted milk price)
	kdensity nm_labu_adj_land if cntry == 1  & year == 2008 & nm_lt1  < 0.3  & nm_lt1  > -0.1, plot(kdensity nm_labu_adj_land if cntry == 2 & year == 2008 & nm_lt1  < 0.3 & nm_lt1  > -0.1) legend(label(2 "NI") label(1 "ROI") rows(1)) scheme(s2color)
	graph export `outdatadir'/kden_nm_labu_adj_land_ROI.pdf, replace  
	* select all farms in 2008 (SE, BMW, NI), compare by country (ROI = SE + BMW to  NI)
	ksmirnov nm_labu_adj_land if year == 2008 & nm_lt1  < 0.3  & nm_lt1  > -0.1, by(cntry) 
	...
	* NM Kernel Density ROI- NI (all exchange rate adjusted)
	...
	graph export `outdatadir'/kden_nm_labu_cex_land_ROI.pdf, replace  
	* select all farms in 2008 (SE, BMW, NI), compare by country (ROI = SE + BMW to  NI)
	ksmirnov nm_labu_adj_land if year == 2008 & nm_lt1  < 0.3  & nm_lt1  > -0.1, by(cntry) 
...
*BMW vs. NI
	* NM Kernel Density (adjusted milk price)
	kdensity nm_labu_adj_land if bmw == 1  & year == 2008 & nm_lt1  < 0.3  & nm_lt1  > -0.1, plot(kdensity nm_labu_adj_land if cntry == 2 & year == 2008 & nm_lt1  < 0.3 & nm_lt1  > -0.1) legend(label(2 "NI") label(1 "BMW") rows(1)) scheme(s2color)
	graph export `outdatadir'/kden_nm_labu_adj_land_BMW.pdf, replace
	* select farms not in SE in 2008 (BMW, NI), compare by country (in effect BMW to NI)
	ksmirnov nm_labu_adj_land if se == 0 & year == 2008 & nm_lt1  < 0.3  & nm_lt1  > -0.1, by(cntry) 
	...
	* NM Kernel Density (all exchange rate adjusted)
	...
	graph export `outdatadir'/kden_nm_labu_cex_land_BMW.pdf, replace
	* select farms not in SE in 2008 (BMW, NI), compare by country (in effect BMW to NI)
	ksmirnov nm_labu_adj_land if se == 0 & year == 2008 & nm_lt1  < 0.3  & nm_lt1  > -0.1, by(cntry) 
*/
